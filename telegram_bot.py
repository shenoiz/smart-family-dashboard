...
def is_allowed(update): return update.effective_user.id in TELEGRAM_ALLOWED_USERS
...
async def cmd_clear(update, ctx):
    ...
    todos = load_todos()
    if who in ('him', 'all'): todos['him'] = [t for t in todos['him'] if not t['done']]
    if who in ('her', 'all'): todos['her'] = [t for t in todos['her'] if not t['done']]
    save_todos(todos)
